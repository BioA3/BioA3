...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between bioa3 and zbioa3
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // BIOA3: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
