// Copyright (c) 2017-2020 The BIOA3 developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zbioa3controldialog.h"
#include "ui_zbioa3controldialog.h"
...
std::set<std::string> ZBioA3ControlDialog::setSelectedMints;
std::set<CMintMeta> ZBioA3ControlDialog::setMints;

bool CZBioA3ControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBioA3ControlDialog::COLUMN_DENOMINATION || column == ZBioA3ControlDialog::COLUMN_VERSION || column == ZBioA3ControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBioA3ControlDialog::ZBioA3ControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBioA3ControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zBIOA3 Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZBioA3->setProperty("cssClass", "text-main-purple");
    ui->labelZBioA3_int->setProperty("cssClass", "text-main-purple");
...
ZBioA3ControlDialog::~ZBioA3ControlDialog()
{
    delete ui;
}

void ZBioA3ControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBioA3ControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBioA3ControlWidgetItem* itemDenom(new CZBioA3ControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBioA3ControlWidgetItem *itemMint = new CZBioA3ControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zBIOA3.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBIOA3 seed used to mint this zBIOA3 is not the same as currently hold in the wallet";
            else
...
void ZBioA3ControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBioA3ControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBioA3_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZBioA3ControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBioA3ControlDialog::GetSelectedMints()
{
...
void ZBioA3ControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
