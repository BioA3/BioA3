...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bioa3 address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BIOA3.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bioa3:URI (bioa3:XyZ...?message=example)
            {
...
bool WalletModel::createZbioa3Spend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZbioa3(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzBIOA3Reset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBIOA3Reset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBIOA3Reset.connect(boost::bind(NotifyzBIOA3Reset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBIOA3Reset.disconnect(boost::bind(NotifyzBIOA3Reset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid BIOA3 address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid BIOA3 address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from BIOA3 address");
...
{
    setMints.clear();
    setMints = pwalletMain->zbioa3Tracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
