...
#include "qt/bioa3/privacywidget.h"
#include "qt/bioa3/forms/ui_privacywidget.h"
#include "qt/bioa3/qtutils.h"
#include "guiutil.h"
#include "qt/bioa3/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(BIOA3GUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBIOA3 anonymizes your BIOA3 by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zBIOA3 or convert back to BIOA3"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BIOA3 ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBIOA3");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBIOA3");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBIOA3");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBIOA3");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBIOA3");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBIOA3");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBIOA3");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBIOA3");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zBIOA3 Movements"));
    ...
    // Only Convert to BIOA3 enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzBIOA3->setTitleClassAndText("btn-title-grey", "Total 0 zBIOA3");
    ui->btnTotalzBIOA3->setSubTitleClassAndText("text-subtitle", "Show denominations of zBIOA3 owned.");
    ui->btnTotalzBIOA3->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BIOA3 outputs to mint into zBIOA3.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zBIOA3");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzBIOA3, SIGNAL(clicked()), this, SLOT(onTotalZbioa3Clicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBIOA3");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BIOA3 to mint into zBIOA3"));
    }else{
        btnText = tr("Convert back to BIOA3");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBIOA3 to convert back into BIOA3"));
    }
...
void PrivacyWidget::onTotalZbioa3Clicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzBIOA3->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBIOA3->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBIOA3 is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zBIOA3, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBIOA3").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBIOA3 minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZbioa3(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zBIOA3 converted back to BIOA3"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBIOA3 </b>";
    ...
    ui->btnTotalzBIOA3->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
