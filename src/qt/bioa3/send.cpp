...
#include "qt/bioa3/send.h"
#include "qt/bioa3/forms/ui_send.h"
#include "qt/bioa3/addnewcontactdialog.h"
#include "qt/bioa3/qtutils.h"
#include "qt/bioa3/sendchangeaddressdialog.h"
#include "qt/bioa3/optionbutton.h"
#include "qt/bioa3/sendconfirmdialog.h"
#include "qt/bioa3/myaddressrow.h"
#include "qt/bioa3/guitransactionsutils.h"
...
#include "zbioa3controldialog.h"

SendWidget::SendWidget(BIOA3GUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("BIOA3");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zBIOA3");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (BIOA3) or private coins (zBIOA3)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("BIOA3 address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 BIOA3");
    ...
    setCssProperty(coinIcon, "coin-icon-bioa3");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onBIOA3Selected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onBIOA3Selected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send BIOA3");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zBIOA3");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZbioa3 = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZbioa3));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZbioa3 ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendBioA3 = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendBioA3 ? AskPassphraseDialog::Context::Send_BIOA3 : AskPassphraseDialog::Context::Send_zBIOA3, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendBioA3) ? send(recipients) : sendZbioa3(recipients)) {
...
bool SendWidget::sendZbioa3(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zBIOA3 is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zBIOA3 selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZBioA3ControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBioA3ControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZbioa3(
        ...
        inform(tr("zBIOA3 transaction sent!"));
        ZBioA3ControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZBIOA3_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zBIOA3 require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zBIOA3 transaction
        ...
        emit message("zBIOA3 transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isBIOA3){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BIOA3 to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZBioA3ControlDialog *zBioA3Control = new ZBioA3ControlDialog(this);
            zBioA3Control->setModel(walletModel);
            zBioA3Control->exec();
            ui->btnCoinControl->setActive(!ZBioA3ControlDialog::setSelectedMints.empty());
            zBioA3Control->deleteLater();
        } else {
            inform(tr("You don't have any zBIOA3 in your balance to select."));
        }
    }
}
...
void SendWidget::onBIOA3Selected(bool _isBIOA3){
    isBIOA3 = _isBIOA3;
    setCssProperty(coinIcon, _isBIOA3 ? "coin-icon-bioa3" : "coin-icon-zbioa3");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress bioa3Add = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(bioa3Add)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(bioa3Add.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
