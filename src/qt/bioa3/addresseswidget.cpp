...
#include "qt/bioa3/addresseswidget.h"
#include "qt/bioa3/forms/ui_addresseswidget.h"
#include "qt/bioa3/addresslabelrow.h"
#include "qt/bioa3/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(BIOA3GUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    ui->btnAddContact->setSubTitleClassAndText("text-subtitle", "Generate a new address to receive BioA3 coins.");
    ...
    // Address
    ui->labelAddress->setText(tr("Enter BIOA3 address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress bioa3Add = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(bioa3Add.Get(), label.toUtf8().constData(),
                bioa3Add.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
