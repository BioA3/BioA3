...
#include "qt/bioa3/dashboardwidget.h"
#include "qt/bioa3/forms/ui_dashboardwidget.h"
#include "qt/bioa3/sendconfirmdialog.h"
#include "qt/bioa3/txrow.h"
...
DashboardWidget::DashboardWidget(BIOA3GUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of BIOA3 staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareBioA3, "square-chart-bioa3");
    setCssProperty(ui->labelSquarezBioA3, "square-chart-zbioa3");
    setCssProperty(ui->labelBioA3, "text-chart-bioa3");
    setCssProperty(ui->labelZbioa3, "text-chart-zbioa3");

    ui->labelZbioa3->setVisible(false);
    ...
    ui->labelAmountZbioa3->setText("0 zBIOA3");
    ui->labelAmountBioA3->setText("0 BIOA3");
    setCssProperty(ui->labelAmountBioA3, "text-stake-bioa3-disable");
    setCssProperty(ui->labelAmountZbioa3, "text-stake-zbioa3-disable");

    ui->labelAmountZbioa3->setVisible(false);
    ...
    ui->btnHowTo->setText(tr("How to get BIOA3"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("BIOA3")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isBioA3 = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZBIOA3;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair BIOA3, zBIOA3
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            bioa3 = (pair.first != 0) ? pair.first / 100000000 : 0;
            zbioa3 = (pair.second != 0) ? pair.second / 100000000 : 0;            
            chartData->totalBioA3 += pair.first;
            chartData->totalZbioa3 += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBioA3.append(bioa3);
        chartData->valueszBioA3.append(zbioa3);

        int max = std::max(bioa3, zbioa3);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("BIOA3");
    set1 = new QBarSet("zBIOA3");
    ...
    set0->append(chartData->valuesBioA3);
    set1->append(chartData->valueszBioA3);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBioA3 > 0 || chartData->totalZbioa3 > 0) {
        setCssProperty(ui->labelAmountBioA3, "text-stake-bioa3");
        setCssProperty(ui->labelAmountZbioa3, "text-stake-zbioa3");
    } else {
        setCssProperty(ui->labelAmountBioA3, "text-stake-bioa3-disable");
        setCssProperty(ui->labelAmountZbioa3, "text-stake-zbioa3-disable");
    }
    forceUpdateStyle({ui->labelAmountBioA3, ui->labelAmountZbioa3});
    ui->labelAmountBioA3->setText(GUIUtil::formatBalance(chartData->totalBioA3, nDisplayUnit));
    ui->labelAmountZbioa3->setText(GUIUtil::formatBalance(chartData->totalZbioa3, nDisplayUnit, true));
