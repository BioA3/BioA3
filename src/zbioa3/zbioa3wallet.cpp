...
CzBIOA3Wallet::CzBIOA3Wallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbioa3 seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBIOA3Seed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBIOA3Seed_deprecated()) {
                    LogPrintf("%s: Updated zBIOA3 seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbioa3 seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbioa3 wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBIOA3Wallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBIOA3Count(nCountLastUsed);
    else if (!walletdb.ReadZBIOA3Count(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBIOA3Wallet::Lock()
{
    seedMaster = 0;
}

void CzBIOA3Wallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBIOA3Wallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBIOA3(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBIOA3Wallet::LoadMintPoolFromDB()
{
...
void CzBIOA3Wallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBIOA3Wallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBIOA3Wallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbioa3Tracker->HasPubcoinHash(pMint.first)) {
...
bool CzBIOA3Wallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBIOA3(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbioa3Tracker which also adds to database
    pwalletMain->zbioa3Tracker->Add(dMint, true);
...
void CzBIOA3Wallet::SeedToZBIOA3(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBIOA3Wallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBIOA3Wallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZBIOA3Count(nCountLastUsed);
...
void CzBIOA3Wallet::GenerateDeterministicZBIOA3(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBIOA3Wallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBIOA3Wallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBIOA3Wallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
