// Copyright (c) 2018-2020 The BIOA3 developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zbioa3/deterministicmint.h>
#include "zbioa3tracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzBIOA3Tracker::CzBIOA3Tracker(std::string strWalletFile)
{
...
CzBIOA3Tracker::~CzBIOA3Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBIOA3Tracker::Init()
{
...
bool CzBIOA3Tracker::Archive(CMintMeta& meta)
{
...
bool CzBIOA3Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBIOA3Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBIOA3Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBIOA3Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBIOA3Tracker::GetSerialHashes()
{
...
CAmount CzBIOA3Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBIOA3Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBIOA3Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBIOA3Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBIOA3Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBIOA3Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBIOA3Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBIOA3Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBIOA3Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBIOA3Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBIOA3Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBIOA3Wallet* zBIOA3Wallet)
{
    bool iszBIOA3WalletInitialized = (NULL != zBIOA3Wallet);
    CMintMeta meta;
    ...
    if (! iszBIOA3WalletInitialized)
        zBIOA3Wallet = new CzBIOA3Wallet(strWalletFile);
    meta.isSeedCorrect = zBIOA3Wallet->CheckSeed(dMint);
    if (! iszBIOA3WalletInitialized)
        delete zBIOA3Wallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBIOA3Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBIOA3Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBIOA3Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBIOA3Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBIOA3Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBIOA3Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBIOA3Wallet* zBIOA3Wallet = new CzBIOA3Wallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBIOA3Wallet);
        }
        delete zBIOA3Wallet;
        LogPrint("zero", "%s: added %d dzbioa3 from DB\n", __func__, listDeterministicDB.size());
    }
