...
- [Development guidelines](#development-guidelines)
    - [General BIOA3 Core](#general-bioa3-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BIOA3 Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BIOA3s (tBIOA3)" on the test network, if you
...
BIOA3 Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bioa3/bioa3/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bioa3
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bioa3 --log_level=test_suite
$ valgrind -v --leak-check=full src/bioa3d -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bioa3.coverage/index.html`.
```
...
- BitcoinMiner : Generates BIOA3s (if wallet is enabled).
...
However, in open source software such as BIOA3 Core, where everyone uses
...
pay attention to for reviewers of BIOA3 Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BIOA3 Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against BIOA3 Core so that they can be integrated
...
file descriptors it will cause problems with BIOA3's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:BIOA3-Project/BIOA3.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bioa3-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bioa3-cli` and the GUI debug console use this table to determine how to
